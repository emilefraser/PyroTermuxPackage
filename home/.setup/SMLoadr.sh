...
set_var()
{
	latest=$(curl http://pastebin.com/raw/mGMEFicV 2> /dev/null); [[ "$latest" =~ [0-9]\.[0-9]\.[0-9] ]] || latest=$(curl https://git.fuwafuwa.moe/SMLoadrDev/SMLoadr/raw/branch/master/VERSION.md 2> /dev/null)
...
instal()
{
	...
	git clone https://git.fuwafuwa.moe/SMLoadrDev/SMLoadr smloadr 2> /dev/null
	cd smloadr
...
selfupdate()
{
	curl https://del.dog/raw/helpme > ./.tmpscript 2> /dev/null
